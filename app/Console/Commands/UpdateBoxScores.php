...
class UpdateBoxScores extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'UpdateBoxScores {--finished} {--all} {--week=0} {--year=} {--seasonTypeInt=} ';
    ...
    public function handle()
    {
        ...
        $finishedPrompt = $this->option('finished');
        $seasonTypeInt = $this->option('seasonTypeInt');

        //Default for "live" game data
        $minutes = 1;
        $count = 0;

        //finished games set data so that we can get it all
        if ($finishedPrompt) {
            $minutes = 10000000;
            $count = 52;				//Set to 52 so that we run this only one time in while loop.
        }

        while ($count < 53) {			//Try to allow to run in only 1 min increments for cron jobs to not overlap
            ...
            // Format as "2019reg", "2019post", etc
            $apiSeasonFormat = $season;
            ...
            if ($allPrompt) {
                ...
                if ($weekPrompt > 0) {  // Handle if the user wants to start a certain week instead of 1.
                    $weekStart = $weekPrompt;
                }
            ...
            try {
                for ($week = $weekStart; $week <= $weekLimit; $week++) {
                    $this->info('UpdateBoxScores: Working on '.$apiSeasonFormat.' week '.$week);

                    // Per SportsData.io We need to purge all previous records and let the system redownload them again.
                    // TODO 2021: Update this so that it only purges the ScoringPlay and ScoringDetail for the specific player_id we are working with further down the code

                    // Background Info:
                    // SportsData can change this data, which generates a NEW unique id. They never tell us the old record is deleted.
                    // It will just be erased from their api/db records. But we already saved the data locally
                    // Since we have a previous record that 'might be invalidated', Per their solution, we need to delete them all and use the new data they provide

                    $this->info('Purge Old Records: Working on '.$apiSeasonFormat.' week '.$week);
                    \App\Models\FantasyData\ScoringPlay::where('season_type', $seasonTypeInt)
                        ->where('season', $season)
                        ->where('week', $week)
                        ->forceDelete();

                    \App\Models\FantasyData\ScoringDetail::where('season_type', $seasonTypeInt)
                        ->where('season', $season)
                        ->where('week', $week)
                        ->forceDelete();

                    $this->info('Done Purge Old Records: Working on '.$apiSeasonFormat.' week '.$week);

                    $result = $apiInstance->boxScoresDeltaV('json', $apiSeasonFormat, $week, $playerstoinclude, $minutes);
                    if (! empty($result)) {
                        foreach ($result as $key=>$val) {
                            ...
                            if (isset($val['scoring_plays']) && count($val['scoring_plays']) > 0) {
                                foreach ($val['scoring_plays'] as $sckey=>$scval) {
                                    $scoring_plays_data = [];
                                    $scoring_plays_data = [
                                    ...
                                    unset($scoring_plays_data);
                                    unset($scval);
                                }
                                unset($result[$key]['scoring_plays']);
                                unset($val['scoring_plays']);
                            }

                            if (isset($val['scoring_details']) && count($val['scoring_details']) > 0) {
                                foreach ($val['scoring_details'] as $sdkey=>$sdval) {
                                    ...
                                    if ($sdval['player_id']) {
                                        ...
                                        $this->info('Missing Data for'.print_r($scoring_details_data));
                                    }

                                    // $this->info("Saved a ScoringDetail model");
                                    unset($scoring_details_data);
                                    unset($sdval);
                                }
                                unset($result[$key]['scoring_details']);
                                unset($val['scoring_details']);
                                // $this->info("unset scoring_details");
                            }

                            if (isset($val['player_games']) && count($val['player_games']) > 0) {
                                foreach ($val['player_games'] as $tkey=>$pval) {
                                    $player_games_data = [];
                                    $player_games_data = [
                                    ...
                                    // $this->info("Saved playerGame model");
                                    unset($player_games_data);

                                    //Calculate fantasy points
                                    //$this->info('Requesitng Points Value for: '.$pval->getName().' - '.$pval->getPosition());
                                    FantasyPointsHelper::calculateFantasyPlayerGameFantasyPoints($week, $season, $seasonTypeInt, $pval->getPlayerId(), $isProjection = false);
                                    ...
                                    //     'week' => $week,
                                    //     'season' => $season,
                                    //     'seasonTypeInt' => $seasonTypeInt,
                                    //     'isProjection' => false
                                    // ];

                                    //Handle TQB data syncing
                                    //Only get players that have played in the game.
                                    if ($pval->getPosition() == 'QB' && $pval->getPlayed()) {
                                        $this->call('SyncPositionGameData', ['--team' => $pval['team'], '--position' => 'TQB', '--week' => $week, '--season' => $season, '--seasonTypeInt' => $seasonTypeInt]);
                                    }

                                    //CalculateAcmeFantasyPointsByGame::dispatch($pval->getPlayerId(), $options);
                                    unset($pval);
                                }
                                unset($result[$key]['player_games']);
                                unset($val['player_games']);
                            }

                            if (isset($val['fantasy_defense_games']) && count($val['fantasy_defense_games']) > 0) {
                                foreach ($val['fantasy_defense_games'] as $fkey=>$fval) {
                                    // $fantasy_scoring_details_data	=	array();
                                    ...
                                    // $this->info('saved FantasyDefenseGame');
                                    unset($fantasy_defense_data);

                                    //Calculate fantasy points
                                    //$this->info('Requesitng Points Value for: '.$fval->getTeam() .' | Game: '. $fval->getGameKey());
                                    FantasyPointsHelper::calculateFantasyPlayerGameFantasyPoints($week, $season, $seasonTypeInt, $fval->getPlayerId(), $isProjection = false);
                                    $options = [
                                        'week' => $week,
                                        'season' => $season,
                                        'seasonTypeInt' => $seasonTypeInt,
                                    ...
                                    //CalculateAcmeFantasyPointsByGame::dispatch($fval->getPlayerId(), $options);

                                    unset($fval);
                                }
                                unset($result[$key]['fantasy_defense_games']);
                                unset($val['fantasy_defense_games']);
                            }

                            if (isset($val['team_games']) && count($val['team_games']) > 0) {
                                foreach ($val['team_games'] as $tkey=>$tval) {
                                    $team_games_data = [];
                                    $team_games_data = [
                                    ...
                                    \App\Models\FantasyData\TeamGame::updateOrCreate(
                                        ['game_key'   => $tval['game_key'], 'team_game_id'   => $tval['team_game_id']],
                                        $team_games_data
                                    );
                                    // $this->info("Saved a TeamGame model");

                                    //Update ST Data
                                    $this->call('SyncPositionGameData', ['--team' => $tval->getTeam(), '--position' => 'ST', '--week' => $week, '--season' => $season, '--seasonTypeInt' => $seasonTypeInt]);
                                    unset($team_games_data);
                                    unset($tval);
                                }
                                unset($result[$key]['team_games']);
                                unset($val['team_games']);
                            }
                            unset($result[$key]);
            ...
            //Handle when to retry loop.
            $endTime = Carbon::now();
            $totalDuration = $endTime->diffInSeconds($startTime);
            if ($totalDuration > 1) {
                $this->info('totalDuration is '.$totalDuration);
                $count += $totalDuration;
            } else {
                sleep(5); //ran for less than a second, 1 minute updates, so we will sleep 5 seconds for next api call. continue loop.
                $count += 5; //per docs suggestions: https://fantasydata.com/developers/implementation-guide
            }
