    ...
    <nav class="navbar navbar-light navbar-expand-md navbar-spark">
        <div class="container-fluid" v-if="user">
            ...
            <div id="navbarSupportedContent" class="collapse navbar-collapse">
				<!-- Header -->
				@include('spark::nav.header')
                <ul class="navbar-nav mr-auto">
                    @includeIf('spark::nav.user-left')
                </ul>

                <a @click="showNotifications" class="notification-pill  mb-md-5 mr-md-0 ml-md-auto">
                ...
                <ul class="navbar-nav">
                    <li class="nav-item dropdown">
                        <a href="#" class="d-block d-md-flex text-center nav-link dropdown-toggle" id="dropdownMenuButton" data-toggle="dropdown"
						   ...
						   @if (Spark::showsTeamSwitcher())
						    ...
						    @if($team->logo_url)
						      <img src="{{$team->logo_url}}" class="dropdown-toggle-image spark-nav-profile-photo">
						   @else
                  ...
                  <img :src="user.photo_url" class="dropdown-toggle-image spark-nav-profile-photo">
						@endif 
					<span class="d-md-block">@{{ user.name }} / @if (Spark::showsTeamSwitcher())
					@foreach (Auth::user()->teams as $team)
					...
					@endif
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
							...
							<div v-if="user.is_commissioner == true">
							...
							<!-- Stop Impersonating -->
                            <a v-if="user.is_manager == true" class="dropdown-item" href="{{route('impersonate.leave')}}">
                                <i class="fas fa-sign-out-alt fa-btn fa-sign-out"></i> {{__('Stop managing team')}}
                            </a>
							
                            <!-- Logout -->
                            <a class="dropdown-item" href="/logout">
                                <i class="fas fa-sign-out-alt text-left fa-btn"></i> {{__('Logout')}}
                            </a>
                        </div>
                    </li>
                </ul>
